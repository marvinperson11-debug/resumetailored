...
// /public/fonts (Latin subsets, generated by scripts/build-fonts.js) and served
// through /fonts.css. Loading them from Google Fonts was a render-blocking
// third-party request chain (googleapis.com stylesheet → gstatic.com woff2) and
// the single biggest FCP cost on mobile — this page has no hero image, so LCP is
// text and font delivery dominates. We rewrite the served HTML rather than edit
// 355 files: every Google Fonts <link> for those three families collapses to one
// self-hosted /fonts.css (+ a preload of the two above-the-fold Latin faces), and
// the now-unused fonts.googleapis/gstatic preconnects are dropped. The signature
// script fonts (Dancing Script / Great Vibes / Satisfy / Caveat) and the
// runtime-built ${sigFont} link are left on Google's CDN untouched — they're a
// niche dashboard feature, not a landing-page cost. There is no CJK webfont to
// self-host: Chinese already renders in the system font stack.
...
function _selfHostFonts(html) {
  ...
  let replaced = false;
  html = html.replace(_GF_LINK_RE, (tag) => {
    if (_FONT_KEEP_ON_CDN.test(tag)) return tag;       // signature/dynamic fonts stay on the CDN
    if (!replaced) { replaced = true; return _SELF_FONT_LINK; } // first self-hostable link → one /fonts.css + preloads
    return '';                                          // drop the rest (covered by /fonts.css)
  });
  // Only strip the Google font preconnects if this page no longer needs them
  // (i.e. it has no remaining Google Fonts link — signature pages still do).
  if (replaced && !/fonts\.googleapis\.com\/css2/.test(html)) html = html.replace(_GF_PRECONNECT_RE, '');
  return html;
}
...
// The custom Google OAuth flow that used to live here was removed: it created a
// second, separate sign-in path that made users authenticate twice (once with
// Google here, then again in the primary sign-in). Google sign-in is now handled
// entirely by the platform's native OAuth (Clerk dashboard providers), which is a
// single one-step redirect. Do not re-add a bespoke Google OAuth route here.
...
// Who may enter the employer portal:
//   'full'    — owner/comp, an employer plan, or an existing employer account.
//   'onboard' — everyone else: may create a FREE employer account (2 lifetime posts).
// The free employer tier is open to ALL — including job-seeker Pro/Lifetime
// subscribers. Only the PAID employer features (Pro/Scale) are gated, by their
// own employer subscription, independent of any job-seeker plan.
...
app.get('/api/employer/status', (req, res) => {
  ...
  res.json({
    isEmployer: !!profile,
    access: employerPortalAccess(email), // 'full' | 'onboard'
