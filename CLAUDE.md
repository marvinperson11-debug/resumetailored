...
- **`remotion/`** — the Remotion project:
  ...
  - `narration.js` — free, no-API-key voiceover. Builds a script from the props (`narrationScript` in `data.js`) and runs a local TTS engine (Piper if `PIPER_BIN`+`PIPER_VOICE` set, else `espeak-ng`/`espeak` on PATH; installed via `nixpacks.toml`), returning a WAV `data:` URL + duration. The composition muxes it via `<Audio>` and extends to fit (`audioDurationInFrames`). Best-effort: no engine / `RESUME_VIDEO_VOICE=off` / any failure ⇒ silent video (the route also retries silently if an audio render fails).
