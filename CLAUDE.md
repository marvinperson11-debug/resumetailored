...
## Career Hub (profession-first career platform)

The Career Hub turns a single **saved profession** into the driver of six tools. A user picks their target profession once; it is stored on their `check_ins` row (`profession_id`, `profession_cat`, `seniority`, `profession_set_at`) and injected into every tool via `getUserProfession(email)`. The taxonomy is a static, git-diffable file (`public/data/professions.json`, 10 categories / ~67 professions; ids reuse the SEO role-page slugs where possible so `resolveProfession()` can deep-link to `/{slug}-resume`).

- **Pure core in `career-hub.js` (CH)** — mirrors the `llms-txt.js`/`resume-writeback.js` pattern: taxonomy resolver, cache-key builders, LLM **prompt builders**, JSON **validators**, quiz **scoring + deterministic option shuffle**, `band`/`cappedBand` gating, JSearch normalization, the `LIMITS` table, and the rule-based dashboard `computeNextSteps`. No DB, no network — unit-tested directly in `test/career-hub.js`.
- **`badge-page.js`** — pure OG-tagged HTML for the public `/badge/:slug` share page (branded footer; a growth loop).
...
- **Follow-ups (2026-08):** badge pages are `noindex` (share assets, not SEO — the OG tags still drive unfurls). "Score my answer" is Pro-only **and** capped at `CH.LIMITS.answerScore.pro` (5) per day. **Job alerts:** an opt-in on the Job Finder (`check_ins.job_alerts`, `GET`/`POST /api/jobs/alerts`) drives a daily digest email — `scripts/job-digest.js` (`npm run career:job-digest`, needs `RAPIDAPI_KEY`+`RESEND_API_KEY`) sends the top 5 new jobs per profession via the pure `CH.buildJobDigestEmail`. **Cache pre-warming:** `scripts/warm-career-cache.js` (`npm run career:warm`, needs `ANTHROPIC_API_KEY`) pre-generates quizzes/interview/scenarios for `CH.TOP_PROFESSIONS` (top 20) so the first real user of a popular role never waits; idempotent (skips what's cached). Both run on an **in-process daily scheduler** (`career-cron.js`, started from `server.js`, off unless `CAREER_CRON=on`) rather than standalone Railway cron services — because both need the web service's local SQLite Volume, which a separate service can't attach to (see `docs/RAILWAY_CRON.md`). **GA:** the frontend fires `quiz_start`/`quiz_complete`/`badge_share`/`job_save`/`gap_analysis_run`/`interview_practice_start` through the site's `gtag`. **i18n (full Chinese):** static UI routes through a `t(key, fallback)` helper with a complete `ch_*` Simplified-Chinese dictionary (`CH_I18N_ZH`) merged into `APP_I18N.zh`; the site's `toggleLang` is wrapped to re-render the open tool. **AI-generated content** can't live in a static dictionary (unique per generation), so the generator is **language-aware**: prompt builders take `lang` and (for `zh`) instruct Simplified-Chinese output, and every content cache key is **namespaced by language** — the Chinese quiz/interview/scenario/gap is generated once and cached cross-user, independently of English. Profession names carry `labelZh` (in `professions.json`; 67 roles + 10 categories); the badge page (`?lang=zh`) and job-digest email are localized, with the user's language stored on `check_ins.lang`. `scripts/warm-career-cache.js` warms `WARM_LANGS` (default `en`; `en,zh` warms both at ~2× credits).
...
| `shared_resumes` | Snapshot resumes behind `/r/:slug` share links (indexable, watermarked footer) |
...
Pro users publish a resume as a live public page (`personal_sites` table; `POST/GET/DELETE /api/personal-site`, Pro-gated via `isSubscriber`). Rendered at **`/site/:name`** — indexable and **watermark-free** — by the shared `_shareResumeHtml(row, origin, opts)` renderer (also used by `/r/:slug` share links, which are indexable and keep the brand footer). Subdomains are validated (3–30 chars, `RESERVED_SUBDOMAINS` blocklist).

**Media limits: 5 videos per site, 2 minutes each, 1 GB total — locked-in policy from the account owner.** This is a deliberate reinstatement of a count cap a previous round removed; see the comment on `MEDIA_LIMITS` in `server.js` for why that isn't a contradiction (every video-kind row still counts, including the text-video generator's own output — no exemption was asked for). Hitting 5 is recoverable, unlike before: the Uploads panel has a real delete on every tile (`cvDeleteMedia()`), and the rejection message says so. **Per site, not per account** — `site_media.site_sub` ties every row to the one site it was uploaded on (migration via `_ensureColumn`; existing rows predate the column, stay NULL, and are excluded from every site-scoped query, which is exactly the fix). It used to be one pool shared across every site an account had ever built, so a video from a template tried once, weeks earlier, silently spent a slot on whatever site was open today — reported directly as "I hit my limit and I haven't uploaded anything." Deleting a site deletes its media with it (`_deleteSiteMedia`, files unlinked and rows dropped); renaming carries media to the new subdomain rather than losing it (`UPDATE site_media SET site_sub = ...` in the rename path of `POST /api/personal-site`); the legacy account-wide `DELETE /api/personal-site` wipes every site's media (`_deleteAllMediaForEmail`). Every route that touches `site_media` — `GET`/`POST /api/site-media`, the Uploads panel, the text-video generator, the voice recorder — now requires a `subdomain`, sourced client-side from `wcSite.subdomain`; `test/media-limits.js` proves cross-site isolation directly (uploads on a second site count zero against the first) and `test/media-site-scope.js` covers the delete/rename/wipe lifecycle. The 2-minute cap is enforced against a clip's **own container-declared duration**, not its file size — read via `@remotion/media-parser`'s `parseMedia` (`_probeVideoDurationSeconds`; a direct dependency, not an incidental one pulled in by the render stack), which parses only the header boxes and is typically single-digit milliseconds even on a 100 MB file. It **fails open**: a file the parser cannot make sense of is `null` duration, never a rejection — the same house rule as MOV ("accepted, not re-encoded, told so") of never punishing a user for an ambiguity that isn't theirs. `MEDIA_MAX_VIDEO_SEC` overrides the 120s default for tests only, so a real ~1s fixture (`test/fixtures/`) can prove a real rejection without recording a genuine two-minute clip on every run. Client-side, `wcUploadMedia` probes a video's `<video>`-element `loadedmetadata` duration **before** starting the (possibly 100 MB, possibly slow) upload — a courtesy that saves the wait, never the enforcement; the server is still the only place this is authoritative. Hitting the count cap emails the owner via `notifyOwner`, deduplicated to once per subscriber per day through `usage_store` (a repeated attempt is not five separate facts). Uploads are **streamed to disk** (`multer.diskStorage`, temp dir at `${DATA_DIR}/site-media/.tmp` so the final step is a `rename` on the same filesystem, not a copy) — measured at 25 MB of server RSS for three concurrent 100 MB uploads. Because the file lands BEFORE any check runs, every rejection must delete it: `bail()` in `POST /api/site-media` is the only exit and always unlinks, and `test/media-upload-disk.js` proves six rejection paths leave the temp dir empty. multer's own `fileSize` is the LARGEST per-kind cap (it cannot know a part's kind before reading it), so the 8 MB image cap is enforced in the route afterwards. The client uploads via `XMLHttpRequest`, not `fetch`, because only XHR reports upload progress and 100 MB without it reads as a hang. `_mediaUsage` reads an empty `kind` from the file's mime and never counts an unidentifiable row as video.

**A phone keeps the layout and scales it.** The published page's `@media(max-width:820px)` block used to force `.sd-el{position:static!important;width:100%!important}`, flattening every design into one column. It now lays the page out at `SD_MOBILE_W` (1000px — the same width the editor's phone-frame preview uses, so published matches previewed) inside `.sd-page` and scales it with `transform:scale(var(--sdk))`; `.sd-vp` carries the scaled height, or the document is 1/k too long. `--sdk` is set by an inline script because `scale()` needs a unitless factor and `calc(100vw/1000)` is a length; it recomputes on resize, orientationchange, `fonts.ready` and a `ResizeObserver`. The trade-off is type size (16px body renders at ~6px on a 390px screen) — lowering `SD_MOBILE_W` enlarges everything but makes boxes relatively taller than drawn, because widths are percentages and heights are pixels.
...
- **The shell's left offset matches the app sidebar's REAL breakpoint, not an unrelated number — and tracks WHERE the sidebar actually is, not just whether it's collapsed.** `.cv-shell` is `position:fixed`, so it does not participate in `.dashboard`'s own `grid-template-columns:240px 1fr` layout — it offsets itself to visually line up with where that grid's sidebar column happens to be, activated by its own media query. That query used to say `min-width:981px`, picked independently of `.sidebar`'s real collapse point (`style.css`, `@media(max-width:900px)`), so between 901 and 980px the sidebar was still a real 240px column while the shell had not yet been pushed past it — the editor's own rail rendered directly under the app's sidebar in that whole window. Reported directly as "the sidebar is cut off, tucking behind the canvas." Both breakpoints are now the same number (901px) on purpose. A second, separate bug lived in the offset ITSELF: it was a flat `left:240px`, which only holds while `.dashboard` starts at the viewport's left edge — but `.dashboard` is `max-width:1600px;margin:0 auto`, so past 1600px wide (an external Mac display, easily) it centers and `.sidebar`'s real x position moves right with it (at 1920px, `.sidebar` sits at x=160..400, not x=0..240). A flat 240px shell then painted its own opaque background over the sidebar's last 64-240px, hiding the FREE/PRO badges and the tail of every label — reported as "the sidebar text is cut off on my Mac" (a wide/external display), a different symptom (content hidden under the shell, not an empty gap) from the 901-980px case. Fixed with `left: max(240px, calc((100vw - 1600px) / 2 + 240px))`, which reproduces `.dashboard`'s own centering math instead of assuming `.sidebar` starts at x=0. `test/browser/sidebar-breakpoint.js` proves both fixes with live geometry from 880px up through a 2560px external display, not just a source-level check that the CSS says the right thing.
...
- **The device toggle is a desktop control.** Hidden below 820px (`.cv-seg--device`, `.wc-tplhead-dev`) AND the device pinned to desktop there — `_pvDeviceLocked()` is read by both previews and by `edFitStage`, and a resize handler snaps `edDevice` back when the window crosses down. Hiding without pinning strands a preview in a state whose control is gone; and "mobile" on a phone lays a 390px page inside a ~334px frame, scaled twice, which is not what is published. The same buttons drove the editor canvas, so the canvas loses its phone view on a phone too.
