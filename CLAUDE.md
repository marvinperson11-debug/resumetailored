...
- **Seniority variants**: `/{level}-{role}-resume` and `/{level}-{role}-cover-letter` where level ∈ `entry-level`/`senior`/`lead`, for the top 10 tech + business roles (software-engineer, product-manager, project-manager, data-analyst, data-scientist, business-analyst, marketing-manager, web-developer, devops-engineer, ux-designer) = 30 variant pages each. Each has level-specific title/keywords/H1/FAQ/sample-opening (unique content so they rank separately from the base role) and a "By experience level" cross-link block to the base + sibling levels.
