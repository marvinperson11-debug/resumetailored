...
- **`remotion/`** — the Remotion project:
  ...
  - `narration.js` — free, no-API-key voiceover. Builds a script from the props (`narrationScript` in `data.js`) and runs a local TTS engine, returning a WAV `data:` URL + duration. Engine: **Piper** (natural neural voice) when a Piper binary (`PIPER_BIN`, `piper` on PATH, or the `python3 -m piper` pip CLI) + a voice model are available — model resolved from `PIPER_VOICE`/common dirs, else best-effort downloaded; **espeak-ng** (robotic) is the fallback. `nixpacks.toml` installs Piper + the `en_US-lessac-medium` voice on Railway (guarded with `|| true`, so the deploy never fails over voice). The composition muxes the audio via `<Audio>` and extends to fit (`audioDurationInFrames`). Best-effort throughout: no engine / `RESUME_VIDEO_VOICE=off` / any failure ⇒ silent video, and the route retries silently if an audio render fails.
