...
- **Preview** (`wcSetView`) is the stage's other view. `wcView` tells `edSetDevice` which one to resize — the toggle used to move only the canvas — and `wcFitPreview` sizes the frame from a MEASURED stage width, because `width:100%` of an auto-width parent falls back to the iframe default of 300px.
