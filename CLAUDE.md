...
**Media limits: storage only, no count cap.** `videoMaxCount` is gone — it counted every video ever stored, including the ones the text-video generator writes on the user's behalf, while the meter read "4/5" and the Uploads panel had no delete, so the message "Delete one first" named an action the app did not have (`DELETE /api/site-media/:id` existed and nothing called it). Now: `videoPool` 1 GB, `imageAudioPool` 300 MB, per-file 25 MB (uploads are buffered in RAM via `multer.memoryStorage`, so raising per-file means switching to disk storage first), a storage message that states the amount used and points at Uploads, and `cvDeleteMedia()` wired to a delete button on every tile. `_mediaUsage` reads an empty `kind` from the file's mime and never counts an unidentifiable row as video.
