...
import { getCertificate } from "@/lib/docusign";

export const runtime = "nodejs";

/**
 * Stream the DocuSign certificate of completion (PDF) for a signed/completed
 * envelope. Owner-scoped: the envelope must belong to the caller's employer.
 */
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const pdf = await getCertificate(
    { baseUri: access.baseUri, accountId: access.accountId, accessToken: access.accessToken },
    env.envelopeId
  );
  if (!pdf) return NextResponse.json({ error: "certificate_unavailable" }, { status: 502 });

  return new NextResponse(new Uint8Array(pdf), {
    status: 200,
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `inline; filename="offer-certificate-${id}.pdf"`,
